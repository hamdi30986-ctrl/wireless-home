    ...
    <div className="min-h-screen bg-[#f4f4f5]">
        ...
        <div className="max-w-5xl mx-auto flex items-center justify-between">
          <div className="flex items-center gap-5">
            ...
            <div>
              ...
              <div className="flex items-center gap-2 mt-1">
                ...
                <p className="text-xs text-gray-400">{project ? `${project.customer_name} • ${project.project_type}` : 'No active project'}</p>
              </div>
            </div>
          </div>
          ...
          <div className="flex items-center gap-6">
            <div className="relative" ref={signOutRef}>
              <button onClick={() => setShowSignOutConfirm(!showSignOutConfirm)} className="text-gray-400 hover:text-white text-sm flex items-center gap-2 transition-colors"><LogOut className="w-4 h-4" /> Sign Out</button>
              <AnimatePresence>
                {showSignOutConfirm && (
                  <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: 10 }} className="absolute right-0 top-full mt-4 w-56 bg-white rounded-2xl shadow-2xl border border-gray-100 p-4 z-50">
                    <p className="text-slate-900 text-xs font-bold mb-4 text-center">Confirm Sign Out?</p>
                    <div className="flex gap-2">
                      <button onClick={handleSignOut} className="flex-1 py-2 bg-[#111318] text-white text-[10px] font-bold rounded-lg hover:bg-black transition-all">SIGN OUT</button>
                      <button onClick={() => setShowSignOutConfirm(false)} className="flex-1 py-2 bg-gray-50 text-slate-500 text-[10px] font-bold rounded-lg border border-gray-200 transition-all">CANCEL</button>
                    </div>
                  </motion.div>
                )}
              </AnimatePresence>
      ...
      <div className="max-w-5xl mx-auto px-6 py-8 space-y-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
              <DashboardCard href="/dashboard/proposals" label="Proposals" desc="Review and accept your smart home offers." icon={<Briefcase className="w-5 h-5" />} color="bg-orange-500" />
              <DashboardCard href="/dashboard/vault" label="Smart Vault" desc="Access secure credentials and digital keys." icon={<Shield className="w-5 h-5" />} color="bg-slate-900" />
              <DashboardCard href="/dashboard/financials" label="Financials" desc="Monitor payments and download tax invoices." icon={<FileText className="w-5 h-5" />} color="bg-green-600" />
              <Link href="/dashboard/inquiries" className="group bg-white p-6 rounded-2xl border border-gray-200 shadow-sm transition-all hover:shadow-md hover:border-gray-300">
                ...
                <div className="flex justify-between items-center mb-1"><h3 className="text-base font-semibold text-slate-900">Track Inquiries</h3><ChevronRight className="w-4 h-4 text-gray-400 group-hover:text-slate-900 transition-colors" /></div>
                {latestInquiry ? (
                  <div className="mt-1"><p className="text-xs text-gray-500 mb-1">Latest: {latestInquiry.type}</p><div className="flex items-center gap-1.5"><span className="w-1.5 h-1.5 rounded-full bg-blue-500" /><span className="text-[11px] font-bold text-slate-900 uppercase tracking-tight">{latestInquiry.status}</span></div></div>
              ...
              <Link href="/dashboard/profile" className="group bg-white p-6 rounded-2xl border border-gray-200 shadow-sm transition-all hover:shadow-md hover:border-gray-300">
                ...
                <div className="flex justify-between items-center mb-1"><h3 className="text-base font-semibold text-slate-900">My Profile</h3><ChevronRight className="w-4 h-4 text-gray-400 group-hover:text-slate-900 transition-colors" /></div>
...
function DashboardCard({ href, label, desc, icon, color }: any) {
    return (
        <Link href={href} className="group bg-white p-6 rounded-2xl border border-gray-200 shadow-sm hover:shadow-md hover:border-gray-300 transition-all">
            ...
            <div className="flex justify-between items-center mb-1"><h3 className="text-base font-semibold text-slate-900">{label}</h3><ChevronRight className="w-4 h-4 text-gray-400 group-hover:text-slate-900 transition-colors" /></div>
